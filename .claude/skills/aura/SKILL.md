...
app.before(ctx -> log.info("{} {}", ctx.method(), ctx.url()))
   .before(Aura.requireAuth()).exclude("/login", "/register", "/health")
   .after(ctx -> log.info("done")).exclude("/health");

// after() 返回 BeforeBuilder，支持 exclude（与 before 完全对称）
app.after(ctx -> ctx.header("X-Time", elapsed())).exclude("/static/*");
...
16. **`@RateLimit(n)` 方法级限流**，value = 请求数，window = 秒（默认 60）
17. **`@Scheduled` 定时任务**，cron/fixedRate/fixedDelay 三选一，启动日志打印任务列表
18. **`findOneOrThrow()` / `findByIdOrThrow()`** → 404，替代手动 null 检查

## Rate Limiting（v0.6.2+）

```java
// 全局：所有端点，dev 模式下禁用
app.rateLimit(100, Duration.ofMinutes(1));

// 方法级：在 @Path service 方法上
@RateLimit(5)           // 5 req/60s per IP
@Post("/login")
...
超限 → 429 + `Retry-After` header + JSON error。

## Scheduled Tasks（v0.6.2+）

```java
public class StatsService {
    @Scheduled(cron = "0 0 * * *")       // 5-field cron
    public void resetDaily() { ... }

    @Scheduled(fixedRate = 60_000)        // 毫秒
    public void refreshCache() { ... }

    @Scheduled(fixedDelay = 30_000)       // 上次完成后
    public void pollQueue() { ... }
}
app.services(StatsService.class);  // 必须注册
```

规则：public + 无参 + cron/fixedRate/fixedDelay 三选一。启动日志打印 "Scheduled tasks:" 列表。
...
db.table("user").where("id", 1).findOneOrThrow();  // null → NotFoundException (404)
db.table("user").where("age", ">", 18).orderBy("name").find();
db.findByIdOrThrow("user", id);  // shortcut
