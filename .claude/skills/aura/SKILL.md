...
```java
// 注册
app.register(new UserService(db));
app.register("mailer", new MailService());

// 获取
app.getBean(UserService.class);
app.getBean("mailer", MailService.class);

// 插件模式
app.plugin(a -> a.register(new RedisClient()));
```

不是自动扫描，不是构造器注入。手动 new 手动注册。
...
11. **全局中间件用 `app.before()` / `app.after()`**，不需要进 routes block
12. **Profile 自动加载** — `aura-{env}.properties` 覆盖 `aura.properties`
13. **`app.props(prefix)` 按前缀获取配置**，解决遍历配置的需求
14. **`ctx.ip()` 获取客户端 IP**，自动处理 X-Forwarded-For
